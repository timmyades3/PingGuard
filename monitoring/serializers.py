...
from .models import Group, IpAddress, Endpoint, HourlyStat, DailyStat, MonitorCheck, Settings
...
class EndpointSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if Endpoint.objects.filter(
            user=user, url=url, http_method=http_method
        ).exists():
            raise serializers.ValidationError(
                "You have already added this endpoint with the same HTTP method."
            )
        return attrs
