...
def master_monitor_check():
    ...
    msg = f"Dispatched {active_ips.count()} IPs and {active_endpoints.count()} Endpoints"
    logger.info(msg)
    return msg
...
def ping_ip(ip_id):
    ...
    if ip.port:
        ...
        try:
            ...
            res = {"ip": ip.ip_address, "port": ip.port, "status": "DOWN",
        ...
        try:    
            ...
            logger.error(f"Failed to create MonitorCheck for IP {ip.ip_address}:{ip.port} - {str(e)}")        
...
def check_endpoint(endpoint_id):
    ...
    try:
        ...
        logger.error(f"Failed to create MonitorCheck for Endpoint {endpoint.url}: {str(e)}")    
        ...
        for entry in aggregated_ip:
            obj, _ = HourlyStat.objects.update_or_create(
                ...
                defaults={
                    ...
                    'uptime_percent': round(entry['up_count'] / entry['total'] * 100, 2)
                }
            )
...
def daily_aggregate_and_cleanup():
    ...
    old_hourly = HourlyStat.objects.filter(minute__lt=cutoff)
    ...
    if ip_groups.exists():
        aggregated_ip = ip_groups.annotate(
            day=TruncMinute('minute')
        ).values('user', 'ip_address', 'day').annotate(
            total_checks=Sum('total_checks'),
            up_count=Sum('up_count'),
            down_count=Sum('down_count'),
            weighted_rt_sum=Sum(F('avg_response_time_ms') * F('total_checks')),
            hours_count=Count('id')
        )

        for entry in aggregated_ip:
            total = entry['total_checks'] or 0
            up = entry['up_count'] or 0
            down = entry['down_count'] or 0
